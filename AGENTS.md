...
- `api/v1beta2/` - the `mirror.plugin.fluxcd.io/v1beta2` API types: the `Config` wire model (`hosts`, `artifacts`, `charts`, selector, verification) and the sync `Report` envelope, annotated with kubebuilder markers. `zz_generated.deepcopy.go` and the published JSON Schemas are generated from these types; the package stays dependency-light (apimachinery + stdlib) and carries only the wire structs, enums, consts, and `Effective*` helpers — no semver/OCI/SPIFFE deps. `convert.go` maps a deprecated `api/v1beta1.Config` onto the current shape.
- `api/v1beta1/` - the deprecated `mirror.plugin.fluxcd.io/v1beta1` wire types, kept so old configs still decode. Frozen: no new fields or behavior, only the conversion in `api/v1beta2/convert.go` reads it.
- `internal/config/` - decoding (`Decode`), semantic validation (`Validate`, `ValidateNoEntriesOK`), and path resolution (`ResolvePaths`) for `api/v1beta2.Config` (`apiVersion: mirror.plugin.fluxcd.io/v1beta2`, `kind: Config`). A v1beta1 document is decoded with `api/v1beta1` and converted to v1beta2 in memory, emitting a migration warning (`DeprecationWarning`); validation accepts both apiVersions. Validation is free functions over the API types, not methods.
- `tools/schema-gen/` - generator that turns controller-gen CRD output into a standalone draft 2020-12 JSON Schema. Run via `make generate`; do not edit `docs/{config,report}/*-v1beta2.json` by hand.
...
- `internal/sync/` - sync runner, retry/timeout behavior, per-entry execution, summaries, outcomes, and exit-code aggregation. Report assembly (`NewReport`, `RenderReport`) builds the `api/v1beta2.Report` envelope; the wire report types live in `api/v1beta2`.
...
- `make generate` - download `controller-gen` if needed, regenerate `api/v1beta2/zz_generated.deepcopy.go`, and regenerate the config/report JSON Schemas under `docs/`. Run after changing any `api/v1beta2` type or marker; CI fails if the tree is left dirty.
...
- Config and report wire types live in `api/v1beta2`; semantic config validation lives in `internal/config`. Do not defer schema or semantic validation to mirror execution if it can be rejected before network operations. After changing an `api/v1beta2` type or kubebuilder marker, run `make generate` and commit the regenerated deepcopy and JSON Schemas.
...
- `docs/config.md` + `docs/config-v1beta2.json` - the single config doc: YAML specification (hosts, artifacts, charts, selectors, overwrite behavior, signature verification, defaults), envelope reference, and generated JSON Schema.
- `docs/report.md` + `docs/report-v1beta2.json` - sync report envelope reference and its generated JSON Schema.
...
- New or changed config field: add it to the `api/v1beta2.Config` types (with kubebuilder markers), run `make generate`, then update `internal/config` validation/tests, `docs/config.md`, and examples when appropriate.
- New or changed report field: add it to the `api/v1beta2` report types, run `make generate`, then update `internal/sync` assembly/tests and `docs/report.md`.
